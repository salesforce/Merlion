...
from merlion.models.ensemble.anomaly import (
    DetectorEnsemble,
...
class TestEvaluateAnomaly(unittest.TestCase):
    ...
    def test_single_model(self):
        ...
        model = IsolationForest(
            IsolationForestConfig(n_estimators=25, max_n_samples=None)
        )

        evaluator = TSADEvaluator(
            model=model,
            config=TSADEvaluatorConfig(train_window=None, retrain_freq="7d"),
        )
        ...
        f1 = evaluator.evaluate(
            ground_truth=self.test_labels, predict=alarms, metric=TSADMetric.F1
        )
        p = evaluator.evaluate(
            ground_truth=self.test_labels, predict=alarms, metric=TSADMetric.Precision
        )
        r = evaluator.evaluate(
            ground_truth=self.test_labels, predict=alarms, metric=TSADMetric.Recall
        )
    ...
    def test_ensemble_no_valid(self):
        ...
        model = self.run_ensemble(
            valid_frac=0.0, expected_precision=9 / 57, expected_recall=9 / 9
        )
    ...
    def test_ensemble_with_valid(self):
        ...
        model = self.run_ensemble(
            valid_frac=0.50, expected_precision=9 / 60, expected_recall=9 / 9
        )
        self.assertSequenceEqual(model.combiner.models_used, [False, True, False])
    ...
    def run_ensemble(self, valid_frac, expected_precision, expected_recall):
        ...
        model0 = WindStats(
            WindStatsConfig(wind_sz=60, transform=TemporalResample("5min"))
        )
        ...
        model = DetectorEnsemble(
            ...
            config=DetectorEnsembleConfig(
                combiner=ModelSelector(metric=TSADMetric.F1, invert=False)
            ),
        )
        train_config = EnsembleTrainConfig(valid_frac=valid_frac)
        train_kwargs = {
            "anomaly_labels": self.train_labels,
