...
def main():
    ...
    ts_generators = [
        ...
        TimeSeriesGenerator(
            f=lambda x: 2 * np.sin(x * 0.1) + np.sin(x * 0.02),
            n=n,
            name="seasonal")
    ]

    ts_list = [generator.generate(return_ts=True)
               for generator in ts_generators]

    # Initialize Anomaly Injection Transforms
    anomalize_kwargs = dict(
        anom_prob=0.002,
        anom_width_range=(20,200),
        alpha=0.5,
    )
