...
class TSADBaseDataset(BaseDataset):
    ...
    def describe(self):
        ...
        print("=" * 80)
        print(
            f"Time series in dataset have average length "
            f"{int(np.mean([len(ts) for ts, md in self]))}."
        )
        print(
            f"Time series in dataset have {len(anom_bds) / len(self):.1f} "
            f"anomalies on average."
        )
        print(
        ...
        print(
            f"Anomalies in dataset have average length "
            f"{pd.Timedelta(np.mean([(tf - t0) for t0, tf in anom_bds]))}."
        )
        print(
